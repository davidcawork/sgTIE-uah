...
for i=1:4
    ...
    for h=1:length(h_aux)
        predicted_class = func_kernel(block_design,h_aux(h),block_test);
...
grid on;
xlim([0 25]);
title('Evoluci\''on del error (Kernel) con $k$-Fold ','interpreter','latex','fontsize',18);
...
min_predicted_class = func_kernel(Design,h_aux(min_index),Test);
